...
def generate_1Dwindow(img_ydim, wintype, option=0):
...
def inter_fft_window(rectE, pixsize = (0.5,5),  wintype = 'guassian'):
    """"
    ...
    for i in range(B):    
        ...
        if wintype == 'hamming':
            hamming_win = generate_1Dwindow(y, wintype)
            win = np.zeros((new_y), dtype = complex)
            win[start:start+y] = hamming_win
        else:
            option = y * 0.90
            win = generate_1Dwindow(new_y, wintype, option)
...
for core in cores:
    ...
    for i in range(len(cores)):
        ...
        sq = envi.envi(r'05X05/Envi05X05')
        sqE = sq.loadall()
    ...
    for p in pix_spacing:
        ...
        #read high resolution (square pixel envi file)
        rect = envi.envi((r'#X05/Envi#X05').replace("#", str(p)))
        pixsize = (0.5,p)
        rectE = rect.loadall()
        ...
        wintype = 'hamming'
        Rect_inter = inter_fft_window(rectE, pixsize, wintype)
